...
function activePreset(key){const base=presets[key];return camera?.aspect<1&&base?.portrait?base.portrait:base;}
function selectView(key,instant=false){
 if(!ready)return;
 const v=activePreset(key);if(!v)return;currentView=key;
...
async function start(){
 try{
  ...
  for(const binding of metadata.viewerViews||[]){
   ...
   const cameraPreset=c=>({label:binding.label,p:c.position,t:c.target,f:c.fov,frustum:c.frustum,presentationFrame:c.presentationFrame,sourceCamera:c.name});
   presets[binding.key]=cameraPreset(authored);
   if(binding.portraitCamera){
    const portrait=metadata.cameras.find(c=>c.name===binding.portraitCamera);if(!portrait)throw new Error('Missing portrait camera: '+binding.portraitCamera);
    presets[binding.key].portrait=cameraPreset(portrait);
   }const button=document.querySelector('[data-view="'+binding.key+'"]');if(button)button.hidden=false;
  }
  ...
  const modelEntry=mobile?metadata.assets?.mobile?.model:metadata.assets?.model||'house.glb';if(!modelEntry)throw new Error('Mobile package unavailable');const modelVersion=mobile?metadata.assets.mobile.revision||'r8m1':metadata.stats.source_sha256.slice(0,16);const modelPromise=loader.loadAsync(assetRoot+modelEntry+'?v='+encodeURIComponent(modelVersion));
