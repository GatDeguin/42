...
export async function createPhotographicPipeline({renderer,scene,camera,controls,viewport,mobile,status}) {
 ...
 function material(old){
  ...
  m.name=old.name+' · óptica 95';m.envMapIntensity=1;
  ...
  if(['glass','glassInner','water'].includes(key)){
   ...
   m.thickness=key==='water'?1.45:.012;m.attenuationColor.setRGB(...(key==='water'?[.24,.74,.79]:[.93,.975,.96]));
 ...
 return {
  ...
  async loadBake(loader){
   const [gltf,texture,report,modelInfo]=await Promise.all([loader.loadAsync('./gi/studio-surfaces.glb'),new RGBELoader().setDataType(THREE.HalfFloatType).loadAsync(new URLSearchParams(location.search).get('gi')==='denoised'?'./gi/studio-irradiance-denoised.hdr':'./gi/studio-irradiance.hdr'),fetch('./gi/bake-report.json').then(r=>r.json()),fetch(new URLSearchParams(location.search).has('previous')?'../assets/model-info.json':'./assets/model-info.json').then(r=>r.json())]);
   report.staleSource=modelInfo.stats.source_sha256!==report.sourceSHA256;
   if(report.staleSource&&!new URLSearchParams(location.search).has('allowStaleGI')){metrics.bakeDisabled='The irradiance atlas belongs to a different source SHA; using original candidate geometry.';return;}
   gltf.scene.updateMatrixWorld(true);gltf.scene.traverse(o=>{if(o.isMesh)bakedSurfaces.set(o.userData.label||o.name,o);});
   texture.channel=1;texture.flipY=false;texture.colorSpace=THREE.LinearSRGBColorSpace;bakedTexture=texture;bakeReport=report;
  },
  textures,prepareMesh(o){
   ...
   if(baked&&!Array.isArray(o.material)){
    ...
    o.geometry=baked.geometry.clone().applyMatrix4(new THREE.Matrix4().copy(o.matrixWorld).invert().multiply(baked.matrixWorld));
    if(!o.geometry.attributes.uv1)throw new Error('Missing UV2 for baked surface '+o.userData.label);
    if(!bakedMaterials.has(o.material.uuid)){
     const m=o.material.clone();m.lightMap=bakedTexture;m.lightMapIntensity=Math.PI;
     m.onBeforeCompile=shader=>{shader.fragmentShader=shader.fragmentShader.replace('#include <lights_fragment_maps>',THREE.ShaderChunk.lights_fragment_maps.replace('iblIrradiance += getIBLIrradiance( geometryNormal );','/* Visible-sky diffuse already included in the baked irradiance. */'));};
     m.customProgramCacheKey=()=> 'gi-visible-sky-v1';m.needsUpdate=true;bakedMaterials.set(o.material.uuid,m);
    }
    o.material=bakedMaterials.get(o.material.uuid);
   }
  ...
  render(now,{moving,cut,selected}){
   ...
   if(now-lastStatus>600){
    ...
    if(bakeReport)status.textContent+=bakeReport.staleSource?' · ensayo GI de fuente anterior':' · GI del estudio';
