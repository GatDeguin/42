...
let renderer, scene, camera, controls, model, metadata, selected=null, selectionBox=null, cameraMove=null, ready=false, currentView='exterior';
...
function setVisibility(){
 ...
 for(const o of originals)if(o.userData.dynamic)o.visible=(!$('roof').checked?!o.userData.roofPart:true)&&($('vegetation').checked||o.userData.category!=='LANDSCAPE');
...
function setDoor(key,value){
 const d=doors.get(key);if(!d)return;d.target=value;
...
function selectView(key,instant=false){
 ...
 if(['exterior','pb','pa','pileta','huerta','acceso'].includes(key))endP.sub(endT).multiplyScalar(Math.max(1,1.10/camera.aspect)).add(endT);
 if(instant||reduced){camera.position.copy(endP);controls.target.copy(endT);camera.fov=v.f;camera.updateProjectionMatrix();controls.update();cameraMove=null;}
 else cameraMove={start:performance.now(),p:camera.position.clone(),t:controls.target.clone(),f:camera.fov,endP,endT,endF:v.f};
 clearSelection();document.body.classList.remove('menu-open');$('menu').setAttribute('aria-expanded','false');
}
function clearSelection(){selected=null;$('selection').hidden=true;if(selectionBox){scene.remove(selectionBox);selectionBox.dispose();selectionBox=null;}}
function choose(o){
 clearSelection();selected=o;const d=metaOf(o);$('selection').hidden=false;$('object-name').textContent=d.label||o.name||'Elemento arquitectónico';
...
function setupBatching(root){
 ...
 root.traverse(o=>{
  ...
  restoreWood(o,data);
  const dynamic=!!doorAncestor(o);o.userData.dynamic=dynamic;originals.push(o);pickable.push(o);
  ...
  o.castShadow=!materials.some(m=>m.transparent)&&data.category!=='LANDSCAPE';o.receiveShadow=true;
  if(dynamic||Array.isArray(o.material)){o.visible=true;return;}
...
async function start(){
 try{
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio,mobile?1.5:2));renderer.setClearColor(0xe8ede8);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;
  renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;
  scene=new THREE.Scene();scene.background=new THREE.Color(0xe8ede8);scene.fog=new THREE.Fog(0xe8ede8,65,150);
  camera=new THREE.PerspectiveCamera(46,1,.035,400);camera.position.set(...presets.exterior.p);
  ...
  scene.add(new THREE.HemisphereLight(0xe8f3ff,0x68745a,1.15));
  const sun=new THREE.DirectionalLight(0xffe1bd,2.3);sun.position.set(-10,28,23);sun.target.position.set(15,0,8);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);sun.shadow.camera.left=-30;sun.shadow.camera.right=30;sun.shadow.camera.top=30;sun.shadow.camera.bottom=-30;sun.shadow.camera.near=.1;sun.shadow.camera.far=85;sun.shadow.bias=-.00025;sun.shadow.normalBias=.035;scene.add(sun,sun.target);
  const ground=new THREE.Mesh(new THREE.PlaneGeometry(300,300),new THREE.MeshStandardMaterial({color:0x8f9b79,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.set(11,-.20,10);ground.receiveShadow=true;scene.add(ground);
  new ResizeObserver(()=>{const w=viewport.clientWidth,h=viewport.clientHeight;renderer.setSize(w,h,false);const oldPortrait=camera.aspect<1;camera.aspect=w/h;camera.updateProjectionMatrix();if(ready&&oldPortrait!==(camera.aspect<1))selectView(currentView,true);}).observe(viewport);
  metadata=await fetch('./assets/model-info.json').then(r=>{if(!r.ok)throw new Error('metadata download failed');return r.json();});
  woodSurfaces=await fetch('./assets/textures/wood-surfaces.json').then(r=>r.json());
  const texLoader=new THREE.TextureLoader();await Promise.all(Object.entries({fabric:'fabric',paver:'paver',poolBasin:'poolTile',road:'cement',terracotta:'brick',oak:'oak',walnut:'walnut',parquet:'parquet'}).map(async([key,file])=>{const t=await texLoader.loadAsync('./assets/textures/'+file+'_albedo.png');t.colorSpace=THREE.SRGBColorSpace;t.flipY=false;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());if(key==='parquet')t.repeat.set(1/(.70*2.8),1/(.70*1.44));webTextures[key]=t;}));const loader=new GLTFLoader().setMeshoptDecoder(MeshoptDecoder);
  const gltf=await loader.loadAsync('./assets/house.glb',e=>{const total=e.total||metadata.stats.downloadBytes;const progress=Math.min(95,e.loaded/total*95);$('progress').style.width=progress+'%';$('loading-text').textContent='Descargando modelo · '+Math.round(progress)+' %';});
  model=gltf.scene;scene.add(model);$('loading-text').textContent='Preparando materiales e interacción…';
  model.traverse(o=>{if(o.userData.doorKey){const spec=metadata.doors.find(d=>d.key===o.userData.doorKey);if(spec)doors.set(spec.key,{node:o,spec,value:0,target:0,closedP:new THREE.Vector3(...spec.closed.position),openP:new THREE.Vector3(...spec.open.position),closedQ:new THREE.Quaternion(...spec.closed.quaternion),openQ:new THREE.Quaternion(...spec.open.quaternion)});}});
  await new Promise(resolve=>requestAnimationFrame(resolve));
  setupBatching(model);ready=true;selectView('exterior',true);$('progress').style.width='100%';$('status').textContent='Modelo listo';$('status-dot').classList.add('ready');$('loading').classList.add('done');
  window.__viewer={get ready(){return ready;},get stats(){return {drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles,doors:doors.size,meshes:originals.length,downloadBytes:metadata.stats.downloadBytes};},selectView,setAllDoors,get camera(){return camera;},get doors(){return doors;},get renderer(){return renderer;},get originals(){return originals;},pick};
  animate();
