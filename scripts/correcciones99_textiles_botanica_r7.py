...
PREFIX='R7B | ';TAG='revision99_textiles_botanica';S=bpy.context.scene
OUT=os.path.join(ROOT,'review99','r7_textiles_botanica');os.makedirs(OUT,exist_ok=True)
...
def mesh(name,verts,faces,mat,uvs=None,layer='upperDetail',smooth=True):
 me=bpy.data.meshes.new(PREFIX+name);me.from_pydata(verts,[],faces);me.update();o=bpy.data.objects.new(PREFIX+name,me);S.collection.objects.link(o);o[TAG]=True;o['source_layer']=layer;o['status']='P';o['proposal']='R7B visual/physical textile and planting refinement; existing architecture retained'
...
def apply():
 ...
 stable=[o for o in S.objects if not original_scope(o) and not o.get(TAG)]
 before=fingerprints(stable);baseline=len(stable)
 for o in list(bpy.data.objects):
  if o.get(TAG):bpy.data.objects.remove(o,do_unlink=True)
 for me in list(bpy.data.meshes):
  if me.users==0 and me.name.startswith(PREFIX):bpy.data.meshes.remove(me)
 ...
 c=json.load(open(os.path.join(ROOT,'scripts','assets','r7b','blanket-drape-cache.json')));N,M=c['grid'];uv=[((i%(N+1))/N*2.06/.265708,(i//(N+1))/M*1.34/.2663) for i in range(len(c['vertices']))]
 ...
 for k,p in enumerate(base):
  ...
  head_taper=1-min(1,max(0,(v-.68)/.12));loft.append(.0016+head_taper*max(0,.018*edge+cushion-crease))
 ...
 for idx,cx in enumerate([15.384,16.256]):
  ...
  def point(u,v,side):
   ...
   loft=[.166,.184][idx]*(1+.065*u-.045*v)
   ...
   top=zcontact+.032+loft*full+(crease-dent+tension)*full**.7
   bottom=zcontact+.029*(1-full)**1.8
 ...
 for idx,cx in enumerate(centres):
  start=len(CREATED);cy=float(rod_y);mat=textile('Prenda adulta '+str(idx),'garment'+str(idx),.20);hem=[4.25,4.32,4.27,4.34,4.26,4.31,4.245][idx];top=5.034;thickness=[.019,.029,.024,.029,.016,.025,.025][idx];angle=math.radians(angles[idx]);rot=Matrix.Rotation(angle,3,'Z');pivot=Vector((cx,cy,0));phase=idx*1.23
  def gp(x,y,z):return tuple(pivot+rot@Vector((x*(.75 if idx==4 else 1),y,z)))
  def surface(a,t):
   width=float(np.interp(t,[0,.075,.16,.43,.8,1],[.046,.207,.195,.178,.19,.184]));depth=thickness*(.54+.46*math.sin(math.pi*t/2))
...
def build_garden():
 ...
 for name,col,rough in [('lechuga',(.16,.30,.052),.54),('pimiento',(.028,.12,.027),.37),('acelga',(.048,.18,.046),.46),('albahaca',(.06,.21,.045),.44)]:
  m=material('Hoja '+name,col,rough);p=next(n for n in m.node_tree.nodes if n.type=='BSDF_PRINCIPLED');p.inputs['Subsurface Weight'].default_value=.035;p.inputs['Subsurface Radius'].default_value=(.0018,.0012,.0005);p.inputs['Coat Weight'].default_value=.06;p.inputs['Coat Roughness'].default_value=.42
 ...
 for bi,(x0,yc) in enumerate([(2.09,-4.20),(5.09,-4.20),(2.09,-7.15),(5.09,-7.15)]):
  ...
  def leaf(base,az,L,W,rise,cup,phase,maturity=1,kind=None):
   nonlocal nleaves;nleaves+=1;kind=bi if kind is None else kind;N=32;K=16;base=Vector(base);direction=Vector((math.cos(az),math.sin(az),0));side=Vector((-math.sin(az),math.cos(az),0));offset=len(allv);mid=[]
   def point(t,s):
    profile=math.sin(math.pi*t)**(.40 if kind in [0,2] else .72);edge=1
    if kind==0:edge=1+.065*math.sin(9*math.pi*t+phase)+.022*math.sin(19*math.pi*t+phase)
    elif kind==2:edge=1+.045*math.sin(11*math.pi*t+phase)
    width=W*.5*profile*edge
    ...
    if kind==0:
     horizontal=L*math.sin(t*math.pi*.72);height=rise*math.sin(math.pi*t*.7)+L*.18*t**3
    elif kind==2:
     horizontal=L*t*(.73-.15*t);height=rise*math.sin(t*math.pi*.78)-L*.025*t*t
    z=height+cup*s*s*math.sin(math.pi*t)+.0014*math.cos(10*math.pi*t+abs(s)*3+phase)*abs(s)*math.sin(math.pi*t)
  ...
  for pi,(cx,cy) in enumerate(centres):
   ...
   if bi in [0,2]:
    ...
    count=26 if bi==0 else 17
    for li in range(count):
     age=1-li/(count-1);az=phase+li*2.399963+.16*math.sin(li*1.37+pi)
     if bi==0:
      L=(.075+.10*age)*scale;W=(.062+.085*age)*scale;rise=(.12-.045*age)*scale;pet=.008+.016*age;stemrise=.008+.010*(1-age)
     else:
      L=(.12+.125*age)*scale;W=(.055+.069*age)*scale;rise=(.27-.15*age)*scale;pet=.035+.033*age;stemrise=.04+.055*age
     ...
     leaf(end,az,L,W,rise,.014 if bi==0 else .009,phase+li*.7,age)
   else:
