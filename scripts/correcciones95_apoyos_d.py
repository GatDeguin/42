...
S['anchors95']=json.dumps(records,ensure_ascii=False);S['review_iteration']='95 / pass3d measured concrete bearings';S['video_render_requires_explicit_approval']=True
...
json.dump({'status':'P/PROPOSED','fixes':['Wall bracket terminates inside masonry with shank','Gutter pads conform to actual underside at each end' ,'Grille reserve follows inclined pipe','68 anchors have 100mm proposed embedment measured from actual concrete; grout conforms across stepped joint; 10mm leveling base closes lateral finish gap'],'balcony_connections':records},open(os.path.join(ROOT,'review95','bearing_details_3d.json'),'w',encoding='utf8'),ensure_ascii=False,indent=2)
bpy.context.preferences.filepaths.save_version=0;bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=os.path.join(ROOT,'output','Casa_de_Campo_95_Pass3d.blend'),compress=True)
print('PASS3D_SAVED_NO_VIDEO',len(records),flush=True)
