...
# Cradle pads follow the actual 0.5% gutter underside at every support vertex.
for o in list(S.objects):
...
# Measure structural support from the actual meshes, including the +3.20/+3.21 joint.
...
for o in posts:
 ...
 side=o.name.startswith('Baranda lateral')
 if side:
  x0,x1=13.0,13.21
 ...
 for floor in [v for v in S.objects if v.name.startswith(('AC95 | acabado ','AC95 | capa base adherida ')) and v.type=='MESH']:
  fa,fb=bounds(floor);ca,cb=bounds(cutter)
  if np.all(np.minimum(fb,cb)-np.maximum(fa,ca)>.0001):cut(floor,cutter)
 bpy.data.objects.remove(cutter,do_unlink=True)
 # Start inside concrete and subtract the three actual slabs: this also creates
 ...
 records.append({'post':o.name,'plate_source_box':[x0,x1,z0,z1,3.254,3.266],'anchor_x':xs,'anchor_z':zs,'embedment_proposed_m':.10,'individual_anchors':anchors,'substrate_top_min':min(v['substrate_top'] for v in anchors),'substrate_top_max':max(v['substrate_top'] for v in anchors),'status':'P / geometry only; engineer must validate load, edge distance, slab reinforcement and selected anchor'})
...
S['anchors95']=json.dumps(records,ensure_ascii=False);S['review_iteration']='95 / pass3c measured concrete bearings';S['video_render_requires_explicit_approval']=True
...
json.dump({'status':'P/PROPOSED','fixes':['Wall bracket terminates inside masonry with shank','Gutter pads conform to actual underside at each end' ,'Grille reserve follows inclined pipe','68 anchors have 100mm proposed embedment measured from actual concrete; grout conforms across stepped joint; 10mm leveling base closes lateral finish gap'],'balcony_connections':records},open(os.path.join(ROOT,'review95','bearing_details_3c.json'),'w',encoding='utf8'),ensure_ascii=False,indent=2)
bpy.context.preferences.filepaths.save_version=0;bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=os.path.join(ROOT,'output','Casa_de_Campo_95_Pass3c.blend'),compress=True)
print('PASS3C_SAVED_NO_VIDEO',len(records),flush=True)
