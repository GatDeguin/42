...
await p.screenshot({path:'D:/2026/42/audit/web_cut_label_02.png'});
report.wood=await p.evaluate(()=>{const seen=new Set(),data=[];for(const o of __viewer.originals){const m=o.material;if(!m.name?.match(/Roble|Nogal|parquet|wood|Madera|madera original/i)||seen.has(m.uuid))continue;seen.add(m.uuid);data.push({name:m.name,map:!!m.map,normal:!!m.normalMap,roughnessMap:!!m.roughnessMap,roughness:m.roughness,color:m.color.toArray()});}return data;});
for(const key of ['mono','acceso','vestidor']){await p.click('[data-view="'+key+'"]');await p.waitForTimeout(1400);await p.screenshot({path:'D:/2026/42/audit/web_'+key+'_independent_02.png'});}
fs.writeFileSync('D:/2026/42/audit/web_detail_checks_02.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));await b.close();