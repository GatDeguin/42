...
const dir=process.env.PHOTO99_ASSET_DIR||'docs/preview99/assets/',reference=JSON.parse(fs.readFileSync(dir+'source-envelopes.json')),info=JSON.parse(fs.readFileSync(dir+'model-info.json'));
const modelFile=process.env.PHOTO99_MODEL_FILE||info.assets?.model||'house.glb';
...
const io=new NodeIO().registerExtensions(ALL_EXTENSIONS).registerDependencies({'meshopt.decoder':MeshoptDecoder}),doc=await io.read(dir+modelFile);
...
function envelope(node){
 ...
 while(stack.length){const n=stack.pop(),mesh=n.getMesh();
// Anonymous glTF primitive nodes belong to this source object. A named source
// child (LCD, handle, etc.) has its own independent row and must not enlarge its parent.
stack.push(...n.listChildren().filter(child=>!child.getExtras().label||child.getExtras().label===(node.getExtras().label||node.getName())));
if(!mesh)continue;const m=n.getWorldMatrix();
