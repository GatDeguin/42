import {addDoorPartMotion} from './photographic99-door-parts.mjs';
...
const document=await io.read('web-tools/.cache/photographic99/house-raw.glb');
...
for(const node of document.getRoot().listNodes()){
 ...
 for(const p of mesh.listPrimitives()){
  ...
  for(let i=0;i<indices.length;i+=3){const r=find(indices[i]);if(!components.has(r))components.set(r,count++);if(components.get(r)%2===0)kept.push(indices[i],indices[i+1],indices[i+2]);}
...
for(const t of document.getRoot().listTextures()){
 ...
 const output=await sharp(original).webp({lossless:true,effort:5}).toBuffer();
 const [before,after]=await Promise.all([sharp(original).ensureAlpha().raw().toBuffer(),sharp(output).ensureAlpha().raw().toBuffer()]);
 if(!before.equals(after)){textureRecodes.push({name:t.getName(),action:'kept original PNG',reason:'WebP altered RGB in transparent pixels',decodedPixelEquality:true});continue;}
 if(output.byteLength<original.byteLength){t.setImage(output).setMimeType('image/webp');textureRecodes.push({name:t.getName(),before:original.byteLength,after:output.byteLength,decodedPixelEquality:true});}
}
fs.writeFileSync('docs/preview99/assets/texture-recoding.json',JSON.stringify(textureRecodes,null,2));
console.log('TEXTURE_RECODE',textureRecodes);
await io.write('docs/preview99/assets/house.glb',document);
const info=JSON.parse(fs.readFileSync('docs/preview99/assets/model-info.json','utf8'));
addDoorPartMotion(document,info);
info.gi={enabled:false,reason:'No source-matched GI approved for this publication'};info.stats.positionQuantizationBits=16;info.stats.sceneCount=document.getRoot().listScenes().length;info.stats.downloadBytes=fs.statSync('docs/preview99/assets/house.glb').size;info.stats.webNodes=document.getRoot().listNodes().length;info.stats.grassTrianglesBefore=grassBefore;info.stats.grassTrianglesAfter=grassAfter;
fs.writeFileSync('docs/preview99/assets/model-info.json',JSON.stringify(info,null,2));
